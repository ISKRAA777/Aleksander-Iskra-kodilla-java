...
// wyszukiwaniem lotów jak:
    ...
    flight.getListOfFlight().stream() // otwierasz steam
                .filter(list-> list.getNameAirPortTo().equals(nameAirPort)) //filtruje się  z listy i sprawdzam czy jakiś
                .forEach(System.out::println);                              //port lotniczy pasuje do przyjmowanego argumentu
    }                                                                       // jesli tak to drukuje się
    ...
    public void searchThrough(String nameAirPort1, String nameAirPort2) {
        Set<Flight> list1 = flight.getListOfFlight().stream()           //otwieram steam
                .filter(list -> list.getNameAirPortFrom().equals(nameAirPort1)) //  tutaj robi się  bardzo podobnie jak w metodzie
                .collect(Collectors.toSet());                                   //searchTo lecz zapisuję sie wynik do setu
